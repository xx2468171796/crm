...
try {
    switch ($action) {
        case 'update':
            ...
            // 检查链接是否存在
            $link = Db::queryOne('SELECT * FROM file_manager_links WHERE customer_id = :cid', ['cid' => $customerId]);
            
            if (!$link) {
                error_log("链接不存在: customer_id=$customerId");
            ...
            if (!has_customer_link_permission($user, $customer, $link)) {
                error_log("权限检查失败: user_id={$user['id']}, customer_id=$customerId");
                echo json_encode([
                    'success' => false, 
                    'message' => '无权限操作此客户',
                    'debug' => [
            ...
            echo json_encode([
                ...
                'message' => '设置已更新',
                'version' => $version,
                'cache_key' => 'file_manager_link_permission_' . $link['id']
            ]);
            break;
        ...
        case 'generate':
            ...
            $existing = Db::queryOne('SELECT * FROM file_manager_links WHERE customer_id = :cid', ['cid' => $customerId]);
            
            if ($existing) {
                echo json_encode(['success' => false, 'message' => '链接已存在，请使用重置功能']);
                exit;
            }
            ...
            Db::execute('INSERT INTO file_manager_links (customer_id, token, enabled, created_at, updated_at) VALUES (:cid, :token, 1, :now, :now)', [
                'cid' => $customerId,
                'token' => $token,
                'now' => $now,
            ]);
        ...
        case 'toggle':
            ...
            Db::execute('UPDATE file_manager_links SET enabled = :status, updated_at = :now WHERE customer_id = :cid', [
                'status' => $newStatus,
                'now' => time(),
                'cid' => $customerId,
            ]);
        ...
        case 'set_password':
            ...
            Db::execute('UPDATE file_manager_links SET password = :pwd, updated_at = :now WHERE customer_id = :cid', [
                'pwd' => $password === '' ? null : encryptLinkPassword($password),
                'now' => time(),
                'cid' => $customerId,
            ]);
        ...
        case 'reset':
            ...
            Db::execute('UPDATE file_manager_links SET token = :token, updated_at = :now WHERE customer_id = :cid', [
                'token' => $newToken,
                'now' => time(),
                'cid' => $customerId,
            ]);
