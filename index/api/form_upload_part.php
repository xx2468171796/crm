<?php
/**
 * 表单附件 - 分片上传代理（解决CORS问题）
 * POST /api/form_upload_part.php
 * 
 * 查询参数:
 * - upload_id: 上传ID（必填）
 * - storage_key: 存储键（必填）
...
try {
    ...
    // 获取预签名URL
    $presignedUrl = $uploadService->getPartPresignedUrl($storageKey, $uploadId, $partNumber);
    ...
    if ($httpCode < 200 || $httpCode >= 300) {
        ...
        error_log('[form_upload_part] S3返回错误: ' . $httpCode . ' - ' . $body);
    ...
    error_log('[form_upload_part] 错误: ' . $e->getMessage());
