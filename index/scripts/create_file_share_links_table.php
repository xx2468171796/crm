...
try {
    ...
    CREATE TABLE IF NOT EXISTS file_share_links (
        ...
        FOREIGN KEY (region_id) REFERENCES share_regions(id) ON DELETE SET NULL
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='文件分享上传链接';
    ";
    
    $pdo->exec($sql);
    echo "✓ file_share_links 表创建成功\n";
    
    // 创建 file_share_uploads 表记录通过分享链接上传的文件
    $sql2 = "
    CREATE TABLE IF NOT EXISTS file_share_uploads (
        ...
        INDEX idx_project_id (project_id),
        FOREIGN KEY (share_link_id) REFERENCES file_share_links(id) ON DELETE CASCADE,
        FOREIGN KEY (project_id) REFERENCES projects(id) ON DELETE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='分享链接上传记录';
    ";
    
    $pdo->exec($sql2);
    echo "✓ file_share_uploads 表创建成功\n";
