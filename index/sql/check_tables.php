...
try {
    ...
    // 直接创建表
    $sql1 = "CREATE TABLE IF NOT EXISTS `daily_tasks` (
    ...
    $pdo->exec($sql1);
    echo "daily_tasks created\n";
    
    $sql2 = "CREATE TABLE IF NOT EXISTS `task_comments` (
    ...
    $pdo->exec($sql2);
    echo "task_comments created\n";
    
    $sql3 = "CREATE TABLE IF NOT EXISTS `work_approvals` (
    ...
    $pdo->exec($sql3);
    echo "work_approvals created\n";
    
    $sql4 = "CREATE TABLE IF NOT EXISTS `work_approval_versions` (
    ...
    $pdo->exec($sql4);
    echo "work_approval_versions created\n";
    
    // 验证
    echo "\n=== Verification ===\n";
    $tables = $pdo->query("SHOW TABLES")->fetchAll(PDO::FETCH_COLUMN);
    foreach (['daily_tasks', 'task_comments', 'work_approvals', 'work_approval_versions'] as $t) {
        echo "$t: " . (in_array($t, $tables) ? "OK" : "NOT FOUND") . "\n";
    }
